  ...
  buildVMCommonConfig =
    ...
    }:
    {
      ...
      config = {
        ...
        boot.kernelPatches =
          if builtins.hasAttr "${pkgs.stdenv.hostPlatform.system}" kernelPatches then
            kernelPatches.${pkgs.stdenv.hostPlatform.system}
          else
            [ ];
        ...
        systemd.tmpfiles.rules = [ "d '/run/state/ssh' - root - - -" ];
        ...
        services.openssh = {
          ...
          hostKeys = [
            {
              path = "/run/state/ssh/ssh_host_ed25519_key";
              type = "ed25519";
            }
          ];
        };
        ...
        system.build.closureInfo = pkgs.closureInfo { rootPaths = [ config.system.build.toplevel ]; };
      ...
      type = lib.types.attrsOf (
        lib.types.submodule {
          options = {
            ...
            vmpkgs = lib.mkOption { type = lib.types.anything; };
            ...
            kvm = lib.mkEnableOption { default = true; };
  ...
  config = lib.mkMerge [
    ...
    }
    {
      systemd.services = lib.mkMerge (
        lib.flip lib.mapAttrsToList config.services.buildVMs (
          name: cfg:
          let
            ...
            netdevextra =
              if cfg.sshListenPort == null then
                null
              else
                ",hostfwd=tcp:0.0.0.0:${toString cfg.sshListenPort}-:22";
          in
          {
            "build-vm@${name}" = {
              script = ''
            ...
            } // (if !cfg.autostart then { } else { wantedBy = [ "multi-user.target" ]; });
          }
        )
      );
    }
