...
let
  ...
  cmdline = pkgs.writeText "cmdline.txt" ''
    ${lib.optionalString (earlycon!="") earlycon} ${lib.optionalString (console!="") console} ip=dhcp root=/dev/nfs nfsroot=192.168.1.2:/export/rpifour2 ro rootwait elevator=deadline init=${rpifour2_system.config.system.build.toplevel}/init isolcpus=3 nfsrootdebug
  '';
